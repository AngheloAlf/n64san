...
echo "Formatting source files. This will take a bit"
echo "Running clang-format..."
clang-format-${FORMAT_VER} ${FORMAT_OPTS} src/**/*.c src/**/*.cpp include/**/*.h
echo "Running clang-tidy..."
clang-tidy ${TIDY_OPTS} src/**/*.c src/**/*.cpp include/**/*.h -- ${COMPILER_OPTS} &> /dev/null
echo "Adding missing final new lines..."
find src/ -type f -name "*.c" -exec bash -c 'add_final_newline "$@"' bash {} +
find src/ -type f -name "*.cpp" -exec bash -c 'add_final_newline "$@"' bash {} +
find include/ -type f -name "*.h" -exec bash -c 'add_final_newline "$@"' bash {} +
